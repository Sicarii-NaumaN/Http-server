...
std::string_view header::get_method(const std::string &method) const {
...
std::string_view header::get_status(const size_t& code) const {
    switch (code) {
        case 200:
...
std::string_view header::get_content(const std::string &type) const {
    if (type == http::plain) {
        return http::plain;
